...
description: 'Get started with the OpenFiles REST API in minutes'
---

# API Quickstart

Get up and running with the OpenFiles REST API in under 5 minutes.
...
<CodeGroup>
```bash cURL
curl -X POST "https://api.openfiles.ai/files" \
...
url = "https://api.openfiles.ai/files"
headers = {
    "Content-Type": "application/json",
    "x-api-key": "oa_your_api_key_here"
}
...
<CodeGroup>
```bash cURL
curl -X GET "https://api.openfiles.ai/files/hello-world.txt" \
  -H "x-api-key: oa_your_api_key_here"
```

```javascript JavaScript
const response = await fetch('https://api.openfiles.ai/files/hello-world.txt', {
  headers: {
    'x-api-key': 'oa_your_api_key_here'
  }
})
...
url = "https://api.openfiles.ai/files/hello-world.txt"
